...
class TwitterDataProvider(DataProvider):
    ...
    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
        ...
        data_path = os.path.join(
            os.environ['MLP_DATA_DIR'], '{0}.txt'.format(which_set))
        ...
        for line in open(data_path,'r', encoding='utf-8'):
            text, category = line.strip().split('\t')           
            x_tweets.append(text)
            y_tweets.append(category)
            
        inputs= np.array(x_tweets)
        targets = np.array(self.to_int(y_tweets))
        # pass the loaded data to the parent class __init__
        super(TwitterDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(TwitterDataProvider, self).next()
        return inputs_batch, targets_batch

    def to_int(self, targets):
        mapping = {'Film&Animation':0,
                   'Sports':1,
                   'Education':2,
                   'Pets&Animals':3,
                   'People&Blogs':4,
