...
experiment_name = "experiment_{}_batch_size_{}_ptembed_{}".format(experiment_prefix,
                                                                   batch_size, pt_embeddings)
#  generate experiment name

rng = np.random.RandomState(seed=seed)  # set seed

train_data = TwitterDataProvider(which_set="train", batch_size=batch_size, rng=rng)
val_data = TwitterDataProvider(which_set="valid", batch_size=batch_size, rng=rng)
test_data = TwitterDataProvider(which_set="test", batch_size=batch_size, rng=rng)

inputs = train_data.inputs
inputs = np.append(inputs, val_data.inputs)
inputs = np.append(inputs, test_data.inputs)

max_document_length = max([len(x.split(" ")) for x in inputs])
vocab_processor = tflearn.preprocessing.VocabularyProcessor(max_document_length)
x = np.array(list(vocab_processor.fit_transform(inputs)))

train_data.inputs = x[:len(train_data.inputs)]
val_data.inputs =x[len(train_data.inputs):len(train_data.inputs)+len(val_data.inputs)]
test_data.inputs =x[len(train_data.inputs)+len(val_data.inputs):]
...
if pt_embeddings:
    vocab, words = construct_vocab(inputs)
    embedding_size = 300
    ...
    for i in range(1, vocab_size):  # loop each word
        ...
        embedding = None
        try:
            embedding = model[word]  # try to get vector:it is an array.
        except Exception:
            embedding = None
...
# Placeholder setup
data_inputs = tf.placeholder(tf.int32, [batch_size, train_data.inputs.shape[1]], 'data-inputs')
...
classifier_network = ClassifierNetworkGraph(input_x=data_inputs, target_placeholder=data_targets,
                                            dropout_rate=dropout_rate, embeddings=word_embedding, max_sent_length=max_document_length,
                ...
                for batch_idx, (x_batch, y_batch) in enumerate(train_data):
                    ...
                    _, c_loss_value, acc = sess.run(
                        ...
                        feed_dict={dropout_rate: dropout_rate_value, data_inputs: x_batch,
                                   data_targets: y_batch, training_phase: True})
            ...
            with tqdm.tqdm(total=total_val_batches) as pbar_val:
                for batch_idx, (x_batch, y_batch) in enumerate(val_data):
                    c_loss_value, acc = sess.run(
                        ...
                        feed_dict={dropout_rate: dropout_rate_value, data_inputs: x_batch,
                                   data_targets: y_batch, training_phase: False})
